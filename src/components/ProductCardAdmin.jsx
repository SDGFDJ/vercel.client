...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  return (
    <div className="w-36 flex-shrink-0 p-4 bg-white rounded shadow">
      {/* Image */}
      <div className="w-full h-36 flex items-center justify-center border mb-2">
        <img
          src={data?.image[0]}
          alt={data?.name}
      ...
      <div className="flex gap-2 mt-2">
        <button
          ...
          className="flex-1 border px-1 py-1 text-sm border-green-600 bg-green-100 text-green-800 hover:bg-green-200 rounded"
        >
          Edit
        </button>
        <button
          onClick={() => setOpenDelete(true)}
          className="flex-1 border px-1 py-1 text-sm border-red-600 bg-red-100 text-red-600 hover:bg-red-200 rounded"
        >
          Delete
        </button>
      </div>
